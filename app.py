...
def get_sparkline_svg(prices, color="green"):
    ...
    return f'<svg width="{w}" height="{h}"><polyline points="{" ".join(pts)}" fill="none" stroke="{color}" stroke-width="2"/></svg>'
...
def fetch_all_sync(sectors, benchmarks):
    ...
    # 🌟 核心优化：一次性下载所有历史数据 (2年)
    full_data = yf.download(all_tickers + list(all_bench), period="2y", interval="1d", group_by='ticker', progress=False)
    # 🌟 下载分时数据 (用于 Sparkline)
    ...
    for b in all_bench:
        try:
            hist = full_data[b].dropna()
            c_last = hist['Close'].iloc[-1]
            c_prev = hist['Close'].iloc[-2]
            b_res[b] = {"chg": ((c_last - c_prev)/prev_c)*100 if 'prev_c' in locals() else ((c_last - c_prev)/c_prev)*100}
            # 修正逻辑：
            b_res[b] = {"chg": ((hist['Close'].iloc[-1] - hist['Close'].iloc[-2]) / hist['Close'].iloc[-2]) * 100}
    ...
    for sec_name, tickers in sectors.items():
        ...
        for t in tickers:
            try:
                ...
                it = intra_data[t].dropna()
                if h.empty: continue
                
                price = h['Close'].iloc[-1]
                day_chg = ((price - h['Close'].iloc[-2]) / h['Close'].iloc[-2]) * 100
                ...
                results.append({
                    ...
                    "total_5d": ((price - h['Close'].iloc[-6])/h['Close'].iloc[-6])*100,
                    "total_144d": ((price - h['Close'].iloc[-145])/h['Close'].iloc[-145])*100 if len(h)>=145 else 0,
                    "total_288d": ((price - h['Close'].iloc[0])/h['Close'].iloc[0])*100 if len(h)>=288 else 0,
                    "history": h.tail(6)
                })
            except: pass
...
# --- 4. UI 渲染 ---
st.title("🏛️ 战略资产监控终端 (Turbo 2.0)")

with st.sidebar:
    if st.button("🚀 强制全量同步", type="primary", use_container_width=True):
    ...
    if all_t:
        ...
        st.session_state.my_notes[e_t] = st.text_area("逻辑", value=st.session_state.my_notes.get(e_t, ""), height=150)
        if st.button("💾 保存"): save_config(); st.success("同步")
...
for i, (name, v) in enumerate(s_strength.items()):
    with s_cols[i]:
        st.markdown(f"<div style='background:#f0f2f6; padding:10px; border-radius:5px; border-left:5px solid {'green' if v['alpha']>0 else 'red'};'><b>{name}</b><br><span style='font-size:1.2rem; color:{'green' if v['avg_chg']>0 else 'red'};'>{v['avg_chg']:+.2f}%</span><br><small>Alpha: {v['alpha']:+.2f}%</small></div>", unsafe_allow_html=True)
                ...
                with st.container(border=True):
                    ...
                    with c2:
                        ...
                        for idx in range(1, 6):
                            with h_cols[idx-1]:
                                d_chg = ((s['history']['Close'].iloc[idx]-s['history']['Close'].iloc[idx-1])/s['history']['Close'].iloc[idx-1])*100
                        ...
                        st.markdown(f"<div style='background:#eee; padding:5px; margin-top:10px;'>5日: {s['total_5d']:+.1f}% | 144日: {s['total_144d']:+.1f}% | 288日: {s['total_288d']:+.1f}%</div>", unsafe_allow_html=True)
