...
def fetch_stock_full_data(ticker, bench_chg):
    try:
        ...
        return {
            ...
            "desc": st.session_state.my_notes.get(ticker.upper(), "💡 暂无笔记，请在左侧侧边栏添加你的投资逻辑。")
        }
    except: return None
...
with st.sidebar:
    ...
    edit_t = st.selectbox("选择股票编辑", options=list(set(all_edit)))
    note = st.text_area("编辑逻辑 (支持长文本)", value=st.session_state.my_notes.get(edit_t, ""), height=150)
    if st.button("保存笔记"):
        if edit_t: st.session_state.my_notes[edit_t] = note; save_config(); st.success("已保存")

# --- 5. 数据抓取 ---
with st.status("同步全球行情...", expanded=False) as status:
