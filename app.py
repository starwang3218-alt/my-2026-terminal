...
# --- 1. 配置管理 ---
CONFIG_FILE = "strategy_terminal_v13.json"
...
# --- 2. 初始化 ---
st.set_page_config(page_title="2026 战略终端 V13", layout="wide")
        ...
        with tabs[i]:
            ...
            for s in stocks:
                with st.container(border=True):
                    ...
                    with c1:
                        # 核心修改：植入隐藏锚点，并设置负偏移防止被顶部导航栏遮挡
                        st.markdown(f"<div id='{s['ticker']}' style='position:relative; top:-70px;'></div>", unsafe_allow_html=True)
    ...
    with st.container(height=800):
        for i, key in enumerate(rank_keys):
            with rt[i]:
                ...
                # 核心修改：使用原生 HTML 极限压缩行间距，并赋予代码超链接穿透能力
                ...
                for j, item in enumerate(sorted_m):
                    ...
                    <div style='display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid #f1f5f9; padding-bottom:4px;'>
                        <a href='#{item['ticker']}' target='_self' style='text-decoration:none; color:#1e293b; font-weight:700; font-size:0.95rem; cursor:pointer;'>
                            {j+1}. {item['ticker']}
                        </a>
