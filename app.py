...
# --- 2. 页面配置 ---
st.set_page_config(page_title="2026 深度行情终端", layout="wide", initial_sidebar_state="expanded")
st_autorefresh(interval=300000, key="prod_refresh") 
st.title("🏛️ 智能行情监控终端 (长线战略版)")
...
def get_real_sector_radar():
    """板块资金雷达 - 实时流向与强度"""
    etfs = {"半导体(SOXX)": "SOXX", "人工智能(AIQ)": "AIQ", "金融(XLF)": "XLF", "能源(XLE)": "XLE", "医疗(XLV)": "XLV", "科技(QQQ)": "QQQ"}
    results = []
...
def fetch_stock_full_data(ticker, bench_chg):
    ...
    try:
        ...
        latest_c, prev_c = float(hist['Close'].iloc[-1]), float(hist['Close'].iloc[-2])
        ...
        for i in range(1, len(subset_5d)):
            ...
            history_5d.append({
                ...
                "close": c,
                "vol": (c * float(subset_5d['Volume'].iloc[i])) / 1e6
            })
            
        return {
            "ticker": ticker, "price": latest_c, "change": today_chg, 
            "rs": today_chg - (float(bench_chg)/5), "history": history_5d,
            ...
            "desc": st.session_state.my_notes.get(ticker.upper(), "💡 暂无笔记，请在侧边栏编辑。")
        }
    except: return None
...
with st.sidebar:
    ...
    nt = st.text_input("股票代码 (如: CRDO, 600519)")
    if st.button("确定添加"):
        if nt and target_s:
            st.session_state.my_sectors[target_s].append(nt.upper()); save_config(); st.rerun()
    st.divider()
    st.subheader("📝 投资笔记")
    all_edit = [t for ts in st.session_state.my_sectors.values() for t in ts]
    edit_t = st.selectbox("选择股票", options=list(set(all_edit)))
    note = st.text_area("核心逻辑", value=st.session_state.my_notes.get(edit_t, ""), height=150)
    if st.button("保存笔记"):
        if edit_t: st.session_state.my_notes[edit_t] = note; save_config(); st.success("已保存")

# 数据抓取状态
with st.status("正在刷新 288 日周期数据...", expanded=False) as status:
...
for idx, sec in enumerate(sectors_data):
    with s_cols[idx]:
        ...
        st.markdown(f"<div style='border:1px solid #ddd; padding:10px; border-radius:10px; text-align:center; background:white;'><div style='font-size:0.8rem; color:#666;'>{sec['name']}</div><div style='font-size:1.1rem; color:{color}; font-weight:900;'>{sec['chg']:+.2f}%</div><div style='font-size:0.7rem; color:#999;'>流向:{sec['flow']:+.1f}</div></div>", unsafe_allow_html=True)
                        ...
                        with c3:
                            ...
                            for idx, day in enumerate(s["history"]):
                                with h_cols[idx]:
                                    ...
                                    st.markdown(f"<div style='background-color:rgba(0,0,0,0.02); border:{border}; padding:8px 4px; border-radius:8px; text-align:center;'><div style='font-size:0.75rem; color:gray;'>{day['date']}</div><div style='color:{color}; font-weight:900; font-size:1.1rem;'>{day['s_chg']:+.1f}%</div><div style='font-size:0.85rem; font-weight:bold;'>${day['close']:.1f}</div><div style='font-size:0.75rem; color:#888;'>额:{day['vol']:.0f}M</div></div>", unsafe_allow_html=True)
                            # 总结条
                            st.markdown(f"<div style='margin-top:10px; padding:10px; background:rgba(0,0,0,0.03); border-radius:8px; border:1px dashed gray; display:flex; justify-content:space-between; align-items:center;'><span>📊 5日: <b>{s['total_5d']:+.2f}%</b> | 144日: <b>{s['total_144d']:+.1f}%</b> | 288日: <b>{s['total_288d']:+.1f}%</b></span><a href='https://tw.tradingview.com/chart/MdN4tzco/?symbol=NASDAQ:{s['ticker']}' target='_blank' style='text-decoration:none; background:#2962FF; color:white; padding:4px 12px; border-radius:20px; font-size:0.8rem; font-weight:bold;'>📈 K线详情</a></div>", unsafe_allow_html=True)
                            with st.expander("📖 深度投资逻辑 & 企业百科"):
                                st.write(s['desc'])
                        with c4:
